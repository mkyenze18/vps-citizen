...
export default function Collection() {
  ...
  function getResources() {
    ...
    axios.get(`${url}/vps/api/v0/evidences`, {
      params: {
        // ID: 12345
        page: page ? page : 1
      }
    ...
    <div className="row" style={{ display: 'block' }}>
      <div className="col-md-12 col-sm-12  ">
        <div className="x_panel">
            ...
            <h2>
              {/* IPRS Persons <small>IPRS Persons of the world</small> */}
            </h2>
            ...
            <table className="table table-hover">
                ...
                {resources.map(item => (
                    <tr key={item.id} onClick={(e) => getResource(item.id, e)}>
                      ...
                      <td>{item.evidence_category.name}</td>
                      <td>{`${item.officer_incharge.iprs_person.first_name} ${item.officer_incharge.iprs_person.last_name}_${item.officer_incharge.service_number}`}</td>
                      <td>{item.evidence_no}</td>
                    </tr>
                ))}
                </tbody>
            </table>
